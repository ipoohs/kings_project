...
findAssocs(fake_dtm, 'presid', 0.4)
findAssocs(fake_dtm, 'peopl', 0.4)
...
findAssocs(true_dtm, 'presid', 0.4)
findAssocs(true_dtm, 'reuter', 0.4)
...
```{r}
data.frame(terms(fake_lda_best, 20))
```
...
```{r}
data.frame(terms(true_lda_best, 20))
```


Frequent terms in both from LDA models
...
```{r}
topic_fake_lda <- tidy(fake_lda_best, matrix = "beta")
...
```{r}
topic_true_lda <- tidy(true_lda_best, matrix = "beta")
...
```{r}
fake_readability <- textstat_readability(fake_text_clean, c("meanSentenceLength","meanWordSyllables", "Flesch.Kincaid", "Flesch"), remove_hyphens = TRUE,
...
```{r}
true_readability <- textstat_readability(fake_text_clean, c("meanSentenceLength","meanWordSyllables", "Flesch.Kincaid", "Flesch"), remove_hyphens = TRUE,
  min_sentence_length = 1, max_sentence_length = 10000,
  intermediate = FALSE)
